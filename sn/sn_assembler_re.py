...
def ProcessCommands(dict, precalcs = None):
	...
	match(dict["CMD"]):
		...
		case "PAIR":
			...
			entry.append(dict["VALUE1"].to_bytes(1, "little"))
			entry.append(dict["VALUE2"].to_bytes(1, "little"))
		case "11":
			entry.append(b"\x11")
			entry.append(bytes.fromhex(dict["DATA"]))
		case "12":
		...
		case "SELECT2":
			...
			for s in range(0, len(dict["LIST"])):
				dict["LIST"][s]["STRING"] = dict["LIST"][s]["STRING"].replace("%%", "`")
		...
		case "TEXT2":
			entry.append(b"\x45")
			dict["STRING"] = dict["STRING"].replace("%%", "`")
			...
			# Break line injection
			new_string = ""
			if (dict["STRING"].count("%N") == 0) and (len(dict["STRING"].replace("лл", "12345678901234567").replace("йй", "12345678901234567").encode("shift_jis_2004")) > (linesize if dict["TYPE"] != 10 else linesize2)):
				string = dict["STRING"].split(" ")
				begin = 0
				for i in range(len(string) + 1):
					temp_string = " ".join(string[begin:i])
					if (len(temp_string.replace("лл", "12345678901234567").replace("йй", "12345678901234567").encode("shift_jis_2004")) > (linesize if dict["TYPE"] != 10 else linesize2)):
						new_string += " ".join(string[begin:i-1]) + "%N"
						begin = i - 1
				new_string += " ".join(string[begin:])
			if (new_string != ""):
				if (dict["TYPE"] != 10 and new_string.count("%N") > 2):
...
for i in range(len(Filenames)):
	...
	try:
		...
		shutil.copyfile("jsons/%s.json" % (Filenames[i]), "Patched/%s.json" % (Filenames[i]))
		file = open("Patched/%s.json" % (Filenames[i]), "r", encoding="UTF-8")
	print("Assembling progress: %.2f%%, %s          " % ((i/(len(Filenames)+1)) * 100, Filenames[i]), end="\r")
	dump = json.load(file)
	file.close()
...
print("Assembling progress: 100%%, %s          " % Filenames[len(Filenames)-1])
print("Creating text count table for Playing Log, file %04d..." % len(Filenames))
OUTPUT = []

try:
	file = open("Patched/0108.json", "r", encoding="UTF-8")
except:
	shutil.copyfile("jsons/0108.json", "Patched/0108.json")
	file = open("Patched/0108.json", "r", encoding="UTF-8")
dump = json.load(file)
file.close()
...
print("---")
print("Creation of sn.bin was finished.")
print("You will find file in 010071C00CBA4000/romfs/")